...
namespace Bottles.Deployment.Commands
{
    ...
    public class CreateAllCommand : FubuCommand<CreateAllInput>
    {
        ...
        {
            var output = input.OutputFlag ?? "deployment/bottles";
            ...
            {
                var manifest = system.LoadFromFile<PackageManifest>(file);
                var folder = Path.GetDirectoryName(file);
                var zipFileName = new CreateBottleInput().GetZipFileName(manifest);
                var zipOutputPath = Path.Combine(output, zipFileName);
